...
+ 因为 j, k 要占用两个元素，所以要留两个元素，循环临界值(list.length - 2)。
+ j ,k 要选取开始和结束的两个位置，获取绝对值，进行比较，如果更接近记录下来，保存为 ans。
+ 移动 k, j 指针。如果三和小于 `target`, k 往前跑，反之 j 往前跑。相等的话直接返回 sum。
```
/**
 * @param {number[]} list
 * @param {number} target
